...
import { listActorShowtimes, listFavoriteActors } from "@/lib/queries";
import { FavoriteToggle } from "@/components/favorite-toggle";
import { Card, CardContent } from "@/components/ui/card";

export const dynamic = "force-dynamic";

export default async function ActorPage({ params }: PageProps<"/actors/[name]">) {
  const { name: encoded } = await params;
  const name = decodeURIComponent(encoded);

  const showtimes = listActorShowtimes(name);
  const isFavorite = listFavoriteActors().includes(name);
      ...
      {showtimes.length === 0 ? (
        <Card>
          <CardContent className="text-muted-foreground py-10 text-center text-sm">
            등록된 회차가 없습니다. 공연 상세에서 캐스팅을 등록하면 여기에 모입니다.
          </CardContent>
        </Card>
      ) : (
        <>
...
function Section({
  ...
  title: string;
  items: { date: string; time: string; show_id: string; show_name: string; facility: string | null }[];
  muted?: boolean;
}) {
  if (items.length === 0) {
    return (
        ...
        {items.map((s) => (
          <li key={`${s.show_id}-${s.date}-${s.time}`} className="flex gap-3 py-2 text-sm">
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="text-muted-foreground block text-xs">{s.facility}</span>
            </span>
          </li>
        ))}
      </ul>
